...
trait FavoriteScope
{
    ...
    {
        // Make sure 'preference' is the same as the relation name in the Product model
        return $query->with(['preference' => function ($query) use ($userId) {
            $query->where('user_id', $userId);
        }]);
    }

    /**
     * Boot the FavoriteScope trait.
     * This will apply the global scope if the user is authenticated.
     */
    public static function bootFavoriteScope()
    {
        static::addGlobalScope('withFavoriteStatus', function (Builder $builder) {
            ...
            if (Auth::check()) {
                ...
                // Apply the 'withFavoriteStatus' scope to the query
                $builder->with(['preference' => function ($query) use ($userId) {
                    $query->where('user_id', $userId);
                }]);
            }
        });
